...
class DatabaseWrapper:
    ...
    def get_wordpack_by_id(self, id):
        cursor = self.connection.cursor(pymysql.cursors.DictCursor)
        sql = "UPDATE wordpack SET status = %s WHERE id = %s"
        cursor.execute(sql)
        result = cursor.fetchone()
        cursor.close()
        return result

    def search_wordpack(self, id, status):
        cursor = self.connection.cursor(pymysql.cursors.DictCursor)
        sql = "DELETE wordpack SET status = %s WHERE id = %s"
...
class Database(DependencyProvider):
    ...
    def __init__(self):
        ...
        config={'host':'127.0.0.1', 'user':'root', 'password':'', 'database':'word_service', 'autocommit':True}
